...
import { CURATED_CRS } from '../utils/crs';
...
export class ImportDataPanel {
  ...
  /** Source CRS chosen in the File tab; 'auto' = detect / assume WGS84. */
  private importCrs = 'auto';
  ...
  constructor(
    ...
    this.fileInput.addEventListener('change', async () => {
      ...
      for (const file of Array.from(files)) {
        await this.importManager.importFile(file, this.importCrs);
      }
  ...
  private renderFileTab(): string {
    return `
      <div class="settings-section">
        ...
        <label class="import-crs-label">Source CRS
          <select id="import-crs-select">
            <option value="auto"${this.importCrs === 'auto' ? ' selected' : ''}>Auto-detect / WGS84</option>
            ${CURATED_CRS.filter(c => c.code !== 'EPSG:4326').map(c =>
              `<option value="${c.code}"${this.importCrs === c.code ? ' selected' : ''}>${c.code} — ${c.label}</option>`
            ).join('')}
          </select>
        </label>
        <p class="settings-hint">Pick a CRS only if your data isn't WGS84 and has no embedded projection (e.g. a bare <code>.shp</code> with no <code>.prj</code>). Zipped shapefiles and GeoJSON with a CRS are detected automatically.</p>
  ...
  private wireTab(container: HTMLElement): void {
    // ---- File tab ----
    container.querySelector<HTMLSelectElement>('#import-crs-select')?.addEventListener('change', e => {
      this.importCrs = (e.target as HTMLSelectElement).value;
    });
